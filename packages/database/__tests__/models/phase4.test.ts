...
describe("Phase 4 Domain Models", () => {
  ...
  it("should insert analytics daily entry", async () => {
    const entry = await db.insert(formAnalyticsDailyTable).values({
      formId: testFormId,
      date: new Date(),
      views: 10,
      submissions: 2,
  ...
  it("should insert form view", async () => {
    const view = await db.insert(formViewsTable).values({
      formId: testFormId,
      ipHash: "testhash123",
  ...
  it("should insert email notification setting", async () => {
    const setting = await db.insert(emailNotificationSettingsTable).values({
      formId: testFormId,
