...
describe("ResponseService", () => {
  ...
  describe("enforceFormOwnership", () => {
    it("should throw FORBIDDEN if user doesn't own form", async () => {
      ...
      await expect(responseService.list("user1", "form1", 1, 10))
        .rejects.toThrowError(new TRPCError({ code: "FORBIDDEN", message: "You do not have permission to access this form" }));
    });
  });
  ...
  describe("list", () => {
    it("should return paginated responses", async () => {
      const { db } = await import("@repo/database");
      (db as any).query.formsTable.findFirst = vi.fn().mockResolvedValue({ id: "form1", creatorId: "user1" });
  ...
  describe("delete", () => {
    it("should delete response and answers", async () => {
      const { db } = await import("@repo/database");
      (db as any).query.formsTable.findFirst = vi.fn().mockResolvedValue({ id: "form1", creatorId: "user1" });
      (db as any).query.formResponsesTable.findFirst = vi.fn().mockResolvedValue({ id: "res1", formId: "form1", isComplete: true });
      db.transaction = vi.fn().mockImplementation(async (cb) => {
        const tx = {
          delete: vi.fn().mockReturnValue({ where: vi.fn() }),
          update: vi.fn().mockReturnValue({ set: vi.fn().mockReturnValue({ where: vi.fn() }) }),
          query: { formsTable: { findFirst: vi.fn().mockResolvedValue({ id: "form1", totalSubmissions: 5 }) } }
        };
        await cb(tx);
  ...
  describe("exportCsv", () => {
    it("should export responses to CSV formatted string", async () => {
      ...
      const expectedHeader = "Response ID,Submitted At,Completion Time (s),IP Hash,User Agent,Name,Fruits";
